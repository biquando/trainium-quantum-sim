...
for n in range(16, 41):
    ...
    np.random.seed(0)
    unitary_mats = scipy.stats.unitary_group(dim=4, seed=0)
    ...
    for _ in range(NGATES):
        ...
        idcs = np.random.choice(range(n), size=2, replace=False)
        qubit_indices.append(idcs)

    gates = zip(unitaries, qubit_indices)
    new_gates = consolidate(gate_size, n, gates)
    print(f'New number of gates: {len(new_gates)}')
    print(f'New ratio: {NGATES/len(new_gates)}')
