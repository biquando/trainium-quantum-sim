...
def kernel(initial_state, U):
    ...
    for i in range(state.shape[1] // UNITARY_SIZE):
        ...
        state_tile_real = torch.t(initial_state[0, idx_start:idx_end, 0:UNITARY_SIZE])
        state_tile_imag = torch.t(initial_state[1, idx_start:idx_end, 0:UNITARY_SIZE])
        ...
        W_real = U_tile_real @ state_tile_real
        W_imag = U_tile_imag @ state_tile_imag
        SASB = (U_tile_real + U_tile_imag) @ (state_tile_real + state_tile_imag)
        state_tile_real = torch.t(W_real - W_imag)
        state_tile_imag = torch.t(SASB - W_real - W_imag)


        # store
        # nl.store(state[0, i_p, i_f], value=state_tile_imag)
        # nl.store(state[1, i_p, i_f], value=state_tile_real)
        state[0, idx_start:idx_end, 0:UNITARY_SIZE] = state_tile_imag
        state[1, idx_start:idx_end, 0:UNITARY_SIZE] = state_tile_real
